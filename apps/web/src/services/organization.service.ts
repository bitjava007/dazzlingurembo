...
export const organizationService = {
  ...
  async getCountries(isActive?: boolean): Promise<Country[]> {
    ...
    const response = await api.get<Country[]>('/organization/countries', { params });
    return response.data;
  },

  async createCountry(dto: CreateCountryDto): Promise<Country> {
    const response = await api.post<Country>('/organization/countries', dto);
    return response.data;
  },

  async updateCountry(id: string, dto: UpdateCountryDto): Promise<Country> {
    const response = await api.patch<Country>(`/organization/countries/${id}`, dto);
    return response.data;
  },
  ...
  async getBranches(params?: { countryId?: string; isActive?: boolean }): Promise<Branch[]> {
    const response = await api.get<Branch[]>('/organization/branches', { params });
    return response.data;
  },

  async createBranch(dto: CreateBranchDto): Promise<Branch> {
    const response = await api.post<Branch>('/organization/branches', dto);
    return response.data;
  },

  async updateBranch(id: string, dto: UpdateBranchDto): Promise<Branch> {
    const response = await api.patch<Branch>(`/organization/branches/${id}`, dto);
    return response.data;
  },
  ...
  async getWarehouses(params?: { branchId?: string; isActive?: boolean }): Promise<Warehouse[]> {
    const response = await api.get<Warehouse[]>('/organization/warehouses', { params });
    return response.data;
  },

  async createWarehouse(dto: CreateWarehouseDto): Promise<Warehouse> {
    const response = await api.post<Warehouse>('/organization/warehouses', dto);
    return response.data;
  },

  async updateWarehouse(id: string, dto: UpdateWarehouseDto): Promise<Warehouse> {
    const response = await api.patch<Warehouse>(`/organization/warehouses/${id}`, dto);
    return response.data;
  },
  ...
  async getWorkshops(params?: { branchId?: string; isActive?: boolean }): Promise<Workshop[]> {
    const response = await api.get<Workshop[]>('/organization/workshops', { params });
    return response.data;
  },

  async createWorkshop(dto: CreateWorkshopDto): Promise<Workshop> {
    const response = await api.post<Workshop>('/organization/workshops', dto);
    return response.data;
  },

  async updateWorkshop(id: string, dto: UpdateWorkshopDto): Promise<Workshop> {
    const response = await api.patch<Workshop>(`/organization/workshops/${id}`, dto);
    return response.data;
  },
